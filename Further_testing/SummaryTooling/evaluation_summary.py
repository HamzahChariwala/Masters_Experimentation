    ...
    def compact_array(match):
        ...
        if '[' in content:
            ...
            if '"layout"' in json_str[max(0, match.start()-20):match.start()]:
                # For layout, handle the entire nested array structure
                # We know this is a 2D array, so we can parse it more directly
                # First extract all inner arrays
                inner_arrays = re.findall(r'\[\s*([^\[\]]+?)\s*\]', content)
                processed_lines = []
                
                # Process each inner array
                for array_content in inner_arrays:
                    # Compact this inner array's content
                    compacted = compact_nested_array(array_content)
                    # Check if this is the last array (no comma after it)
                    if array_content == inner_arrays[-1]:
                        processed_lines.append(f"[{compacted}]")
                    else:
                        processed_lines.append(f"[{compacted}],")
