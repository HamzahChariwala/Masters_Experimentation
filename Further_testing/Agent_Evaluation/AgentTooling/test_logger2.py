import os
import sys
import json
import numpy as np
from typing import Dict, Any, List, Tuple

# Add the root directory to sys.path to ensure proper imports
project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
sys.path.insert(0, project_root)

# Mock implementation of the key functionality for testing
class TestAgentLogger:
    """
    A simplified agent logger for testing the output format.
    """
    
    def __init__(self, env_id: str, seed: int):
        self.env_id = env_id
        self.seed = seed
        self.env_tensor = self._create_test_env_tensor()
        self.all_states_data = {}
        
        # Generate test data for all valid states - limiting to a small number for testing
        test_positions = [(1, 1), (5, 5), (9, 1), (1, 9)]
        orientations = [0, 1, 2, 3]
        
        # Generate data for specific positions/orientations to keep the output manageable
        for pos in test_positions:
            x, y = pos
            for orientation in orientations:
                state_key = f"{x},{y},{orientation}"
                self._generate_state_data(x, y, orientation, state_key)
    
    def _create_test_env_tensor(self):
        """Create a test environment tensor"""
        env_tensor = np.full((11, 11), "floor", dtype=object)
        
        # Add walls around the perimeter
        env_tensor[0, :] = "wall"
        env_tensor[-1, :] = "wall"
        env_tensor[:, 0] = "wall"
        env_tensor[:, -1] = "wall"
        
        # Add some lava
        env_tensor[3, 5] = "lava"
        env_tensor[5, 3] = "lava"
        env_tensor[5, 7] = "lava"
        env_tensor[7, 5] = "lava"
        
        # Add goal at the bottom right
        env_tensor[9, 9] = "goal"
        
        return env_tensor
    
    def _generate_state_data(self, x: int, y: int, orientation: int, state_key: str):
        """Generate test data for a single state"""
        # Generate a sample path
        path_taken = [state_key]
        
        # Add some steps to the path
        curr_x, curr_y, curr_dir = x, y, orientation
        steps = []
        
        # Create first step data with model inputs
        next_step = {
            "action": 1,
            "target_state": f"{x},{y},{(orientation + 1) % 4}",
            "type": "floor",
            "risky_diagonal": False
        }
        
        first_step = {
            "state": state_key,
            "action": 1,  # Sample action (turn right)
            "reward": 0,
            "terminated": False,
            "truncated": False,
            "cell_type": self.env_tensor[y, x],
            "next_step": next_step,
            "model_inputs": {
                "image": np.random.rand(7, 7, 3).tolist(),
                "flattened_vector": np.random.rand(10).tolist(),
                "MLP_Keys": {
                    "key1": np.random.rand(5).tolist(),
                    "key2": np.random.rand(5).tolist()
                }
            }
        }
        
        steps.append(first_step)
        
        # Add a few more steps
        for i in range(5):
            action = i % 3  # Mix of actions
            
            # Update direction based on action
            if action == 0:  # Left turn
                curr_dir = (curr_dir - 1) % 4
            elif action == 1:  # Right turn
                curr_dir = (curr_dir + 1) % 4
            elif action == 2:  # Forward
                # Move forward based on current direction
                dx, dy = 0, 0
                if curr_dir == 0:  # Right
                    dx, dy = 1, 0
                elif curr_dir == 1:  # Down
                    dx, dy = 0, 1
                elif curr_dir == 2:  # Left
                    dx, dy = -1, 0
                elif curr_dir == 3:  # Up
                    dx, dy = 0, -1
                
                # Check if the move is valid
                new_x, new_y = curr_x + dx, curr_y + dy
                if (0 < new_x < self.env_tensor.shape[1] - 1 and 
                    0 < new_y < self.env_tensor.shape[0] - 1 and 
                    self.env_tensor[new_y, new_x] != "wall"):
                    curr_x, curr_y = new_x, new_y
            
            # Update path
            new_state_key = f"{curr_x},{curr_y},{curr_dir}"
            if new_state_key != path_taken[-1]:
                path_taken.append(new_state_key)
            
            # Get cell type
            cell_type = self.env_tensor[curr_y, curr_x]
            
            # Determine termination
            terminated = cell_type == "goal"
            truncated = False
            reward = 1.0 if terminated else (-1.0 if cell_type == "lava" else 0.0)
            
            # Create step data
            step_data = {
                "state": new_state_key,
                "action": action,
                "reward": reward,
                "terminated": terminated,
                "truncated": truncated,
                "cell_type": cell_type,
                "next_step": {
                    "action": action,
                    "target_state": f"{curr_x},{curr_y},{curr_dir}",
                    "type": cell_type,
                    "risky_diagonal": False
                },
                "model_inputs": {
                    "image": np.random.rand(7, 7, 3).tolist(),
                    "flattened_vector": np.random.rand(10).tolist(),
                    "MLP_Keys": {
                        "key1": np.random.rand(5).tolist(),
                        "key2": np.random.rand(5).tolist()
                    }
                }
            }
            
            steps.append(step_data)
            
            if terminated or truncated:
                break
        
        # Calculate summary statistics
        total_reward = sum(step["reward"] for step in steps)
        path_length = len(path_taken) - 1
        lava_steps = sum(1 for step in steps if step["cell_type"] == "lava")
        success = any(step["terminated"] and step["reward"] > 0 for step in steps)
        
        summary_stats = {
            "total_reward": total_reward,
            "path_length": path_length,
            "lava_steps": lava_steps,
            "success": success,
            "truncated": any(step["truncated"] for step in steps),
            "terminated": any(step["terminated"] for step in steps),
            "into_wall": False,
            "cyclic_rotation": False,
            "reachable": success or path_length > 0
        }
        
        # Store the data in the same format as Dijkstra's algorithm output
        self.all_states_data[state_key] = {
            "standard": {
                "path_taken": path_taken,
                "next_step": next_step,
                "summary_stats": summary_stats,
                "steps": steps
            }
        }
    
    def export_to_json(self, output_path: str = None) -> str:
        """
        Export the test data to a JSON file.
        
        Args:
            output_path (str, optional): Path to save the JSON file.
            
        Returns:
            str: Path to the saved JSON file
        """
        # Build the output data structure to match Dijkstra log format
        output_data = {
            "environment": {
                "layout": self.env_tensor.tolist(),
                "legend": {
                    "wall": "Wall cell - impassable",
                    "floor": "Floor cell - normal traversal",
                    "lava": "Lava cell - avoided or penalized",
                    "goal": "Goal cell - destination"
                }
            },
            "states": self.all_states_data
        }
        
        # Determine output path if not provided
        if output_path is None:
            # Create Agent_Evaluation/AgentLogs directory if it doesn't exist
            agent_logs_dir = os.path.join(
                os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 
                "AgentLogs"
            )
            os.makedirs(agent_logs_dir, exist_ok=True)
            
            # Save to Agent_Evaluation/AgentLogs directory
            output_path = os.path.join(
                agent_logs_dir, 
                f"{self.env_id}-{self.seed}.json"
            )
        
        # Save the data to a JSON file
        with open(output_path, 'w') as f:
            json.dump(output_data, f, indent=2)
            
        print(f"Test data exported to: {output_path}")
        return output_path

def run_test():
    """Run the test"""
    print("Generating test data for agent evaluation output format...")
    
    # Create the test logger
    logger = TestAgentLogger("TestEnv", 42)
    
    # Export the results
    output_path = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
        "AgentLogs",
        "agent_format_test.json"
    )
    
    # Export to JSON
    logger.export_to_json(output_path)
    print(f"\nExported results to: {output_path}")
    
    # Load and validate the JSON structure
    with open(output_path, 'r') as f:
        data = json.load(f)
    
    # Now load the Dijkstra log for comparison if it exists
    try:
        dijkstra_path = os.path.join(
            project_root,
            "Behaviour_Specification", 
            "Evaluations",
            "MiniGrid-LavaCrossingS11N5-v0-42.json"
        )
        
        with open(dijkstra_path, 'r') as f:
            dijkstra_data = json.load(f)
            
        print("\nComparing with Dijkstra log structure...")
        compare_with_dijkstra(data, dijkstra_data)
    except Exception as e:
        print(f"\nCould not compare with Dijkstra logs: {e}")
    
    # Validate output structure
    validate_output(data, output_path)
    
    print("\nTest completed successfully.")

def compare_with_dijkstra(agent_data, dijkstra_data):
    """Compare the agent output structure with the Dijkstra log structure"""
    # Check top-level keys
    agent_keys = set(agent_data.keys())
    dijkstra_keys = set(dijkstra_data.keys())
    
    print(f"Top-level keys match: {agent_keys == dijkstra_keys}")
    
    # Check environment structure
    agent_env_keys = set(agent_data["environment"].keys())
    dijkstra_env_keys = set(dijkstra_data["environment"].keys())
    
    print(f"Environment keys match: {agent_env_keys == dijkstra_env_keys}")
    
    # Check state structure for a specific state if it exists in both
    common_states = set(agent_data["states"].keys()) & set(dijkstra_data["states"].keys())
    if common_states:
        sample_state = next(iter(common_states))
        print(f"\nComparing structure for state {sample_state}...")
        
        # In Dijkstra logs, the state contains 'standard' and possibly other keys
        # In our agent logs, the state contains 'standard'
        agent_state_keys = set(agent_data["states"][sample_state].keys())
        dijkstra_state_keys = set(dijkstra_data["states"][sample_state].keys())
        
        print(f"Agent state keys: {agent_state_keys}")
        print(f"Dijkstra state keys: {dijkstra_state_keys}")
        
        # Check 'standard' vs 'standard' structure
        if "standard" in agent_state_keys and "standard" in dijkstra_state_keys:
            agent_standard_keys = set(agent_data["states"][sample_state]["standard"].keys())
            dijkstra_standard_keys = set(dijkstra_data["states"][sample_state]["standard"].keys())
            
            shared_keys = agent_standard_keys & dijkstra_standard_keys
            print(f"Shared keys between agent standard and dijkstra standard: {shared_keys}")
            print(f"Keys only in agent standard: {agent_standard_keys - dijkstra_standard_keys}")
            print(f"Keys only in dijkstra standard: {dijkstra_standard_keys - agent_standard_keys}")
            
            # Check summary_stats keys
            if "summary_stats" in agent_standard_keys and "summary_stats" in dijkstra_standard_keys:
                agent_stats_keys = set(agent_data["states"][sample_state]["standard"]["summary_stats"].keys())
                dijkstra_stats_keys = set(dijkstra_data["states"][sample_state]["standard"]["summary_stats"].keys())
                
                print(f"Summary stats keys in agent standard: {agent_stats_keys}")
                print(f"Summary stats keys in dijkstra standard: {dijkstra_stats_keys}")
                print(f"Keys in both: {agent_stats_keys & dijkstra_stats_keys}")
    else:
        print("No common states found for direct comparison")

def validate_output(data, output_path):
    """Validate the output data structure"""
    print("\nValidating output structure:")
    
    # Check if the structure matches what we expect
    if "environment" in data and "states" in data:
        print("✓ JSON has correct top-level structure")
    else:
        print("✗ JSON is missing required top-level keys")
    
    if "layout" in data["environment"] and "legend" in data["environment"]:
        print("✓ Environment data is correctly structured")
    else:
        print("✗ Environment data is missing required keys")
    
    # Print summary of structure
    print("\nJSON Structure Summary:")
    print(f"- Total size: {os.path.getsize(output_path)} bytes")
    print(f"- Environment layout dimensions: {len(data['environment']['layout'])}x{len(data['environment']['layout'][0])}")
    print(f"- Number of states: {len(data['states'])}")
    
    # Print stats for a sample state
    if data["states"]:
        sample_state_key = next(iter(data["states"].keys()))
        print(f"\nSample State {sample_state_key}:")
        
        # Check agent data
        if "standard" in data["states"][sample_state_key]:
            agent_data = data["states"][sample_state_key]["standard"]
            
            if "path_taken" in agent_data:
                path = agent_data["path_taken"]
                print(f"- Path taken: {path[:2]}...{path[-2:] if len(path) > 2 else path} (length {len(path)})")
            
            if "next_step" in agent_data:
                print(f"- Next step: {agent_data['next_step']}")
            
            if "summary_stats" in agent_data:
                print(f"- Summary stats: {agent_data['summary_stats']}")
            
            if "steps" in agent_data and agent_data["steps"]:
                first_step = agent_data["steps"][0]
                print(f"- First step keys: {list(first_step.keys())}")
                
                if "model_inputs" in first_step:
                    print(f"- Model inputs keys: {list(first_step['model_inputs'].keys())}")
                    
                    # Validate model inputs
                    for key, value in first_step["model_inputs"].items():
                        if isinstance(value, dict) and value:
                            print(f"  - {key}: dictionary with {len(value)} keys")
                        elif isinstance(value, list) and value:
                            if isinstance(value[0], list) and isinstance(value[0][0], list):
                                print(f"  - {key}: 3D array of shape {len(value)}x{len(value[0])}x{len(value[0][0])}")
                            elif isinstance(value[0], list):
                                print(f"  - {key}: 2D array of shape {len(value)}x{len(value[0])}")
                            else:
                                print(f"  - {key}: 1D array of length {len(value)}")

if __name__ == "__main__":
    run_test() 