...
def create_cumulative_experiments(sorted_experiments: List[Tuple[str, float]], 
    ...
    cumulative_data = {
        ...
        "description": "Cumulative experiments ordered by mean metric value (highest to lowest)",
        "original_experiment_count": len(sorted_experiments),
        "experiments": {}
    }
    
    # Create cumulative experiments
    for i, (exp_name, mean_score) in enumerate(sorted_experiments, 1):
        # Get all experiments up to this point
        cumulative_exp_names = [exp[0] for exp in sorted_experiments[:i]]
        
        # Calculate cumulative mean (simple average for now)
        cumulative_mean = sum(exp[1] for exp in sorted_experiments[:i]) / i
        ...
        experiment_data = {
            ...
            "highest_individual_score": sorted_experiments[0][1],
            "lowest_individual_score": sorted_experiments[i-1][1],
            "individual_scores": [exp[1] for exp in sorted_experiments[:i]]
        }
...
def process_filtered_file(input_file: Path, output_dir: Path) -> bool:
    """
    ...
    try:
        ...
        # Create cumulative experiments
        cumulative_data = create_cumulative_experiments(sorted_experiments, data)
        ...
        print(f"  {input_file.name} -> {output_name} ({len(sorted_experiments)} experiments)")
        return True
...
def generate_cumulative_experiments(agent_path: str, input_files: List[str] = None) -> None:
    """
    ...
    print(f"Processing {len(files_to_process)} filtered files:")
    
    successful = 0
    for file_path in sorted(files_to_process):
        if process_filtered_file(file_path, circuit_verification_dir):
            successful += 1
...
def main():
    ...
    if args.agent_path:
        ...
        generate_cumulative_experiments(args.agent_path)
        ...
        try:
            ...
            print(f"Generated {len(cumulative_data['experiments'])} cumulative experiments")
        except Exception as e:
