#!/usr/bin/env python3
"""Extract comprehensive Optuna plots including parameter importance and optimization history."""
...
# Paths to all four optimization runs
...
RESULTS_20250526_A = "../Agent_Storage/Hyperparameters/optimization_20250526_013800/optimization_results.json" 
RESULTS_20250526_B = "../Agent_Storage/Hyperparameters/optimization_20250526_223042/optimization_results.json"
...
def load_all_optimization_data():
    """Load data from all four optimization runs."""
    print("✓ Loading data from all optimization runs...")
    ...
    # Load 20250526_A (consolidated JSON)
    trials_20250526_a = load_consolidated_results(
        RESULTS_20250526_A, trial_offset=2000, run_name="20250526_A (consolidated)"
    )
    all_trials.extend(trials_20250526_a)
    
    # Load 20250526_B (consolidated JSON)
    trials_20250526_b = load_consolidated_results(
        RESULTS_20250526_B, trial_offset=3000, run_name="20250526_B (consolidated)"
    )
    all_trials.extend(trials_20250526_b)
...
def extract_magma_contour_maps(study, contour_dir):
    ...
    for obj_name, obj_func, obj_title in objectives:
        for param1, param2 in combinations(param_names, 2):
            try:
                ...
                fig.update_layout(
                    title=f"Comprehensive Magma Heat Map: {clean_param1} vs {clean_param2} ({obj_title})",
                    width=1000, height=800, 
                    showlegend=False,
                    coloraxis=dict(colorscale='magma')
                )
                
                filename = f"comprehensive_magma_{obj_name}_{clean_param1}_vs_{clean_param2}.png"
    ...
    print(f"Generated {count} comprehensive contour maps")

def extract_slice_plots(study, slice_dir):
    """Extract comprehensive slice plots."""
    print("Extracting comprehensive slice plots...")
    ...
    for obj_name, obj_func, obj_title in objectives:
        for param_name in param_names:
            try:
                ...
                fig.update_layout(
                    title=f"Comprehensive Slice: {clean_param} vs {obj_title}",
                    width=1000, height=700
                )
                
                filename = f"comprehensive_slice_{obj_name}_{clean_param}.png"
    ...
    print(f"Generated {count} comprehensive slice plots")

def create_parallel_plots(study, parallel_dir):
    """Create comprehensive parallel coordinate plots with magma."""
    print("Creating comprehensive parallel coordinate plots with magma...")
    ...
    for obj_name, obj_func, obj_title in objectives:
        try:
            ...
            fig.update_layout(
                title=f"Comprehensive Magma Parallel: {obj_title}",
                width=1600, height=1000,
                font=dict(size=16), title_font=dict(size=20),
                coloraxis=dict(colorscale='magma')
            )
            
            filename = f"comprehensive_parallel_{obj_name}.png"
    ...
    print(f"Generated {count} comprehensive parallel plots")

def create_parameter_importance_plots(study, importance_dir):
    """Create comprehensive parameter importance plots with magma."""
    print("Creating comprehensive parameter importance plots with magma...")
    ...
    for obj_name, obj_func, obj_title in objectives:
        try:
            ...
            # Apply magma colorscale to bars
            fig.update_traces(
                marker=dict(
                    colorscale='magma',
                    color=[i for i in range(len(fig.data[0].y))],
                    colorbar=dict(title="Parameter Rank")
                ),
            ...
            fig.update_layout(
                title=f"Comprehensive Parameter Importance: {obj_title}",
                width=1000, height=800,
            ...
            filename = f"comprehensive_importance_{obj_name}.png"
    ...
    print(f"Generated {count} comprehensive parameter importance plots")

def create_optimization_history_plots(study, history_dir):
    """Create comprehensive optimization history plots with magma."""
    print("Creating comprehensive optimization history plots with magma...")
    ...
    for obj_name, obj_func, obj_title in objectives:
        try:
            ...
            fig.update_layout(
                title=f"Comprehensive Optimization History: {obj_title}",
            ...
            filename = f"comprehensive_history_{obj_name}.png"
    ...
    print(f"Generated {count} comprehensive optimization history plots")

def main():
    print("EXTRACTING COMPREHENSIVE OPTUNA PLOTS (ALL 4 OPTIMIZATION RUNS)")
    print("Including Parameter Importance & Optimization History with Magma Colors")
    print("=" * 80)
    ...
    print("✓ Creating comprehensive study...")
    study = create_comprehensive_study(all_trials, config)
    total_trials = len(study.trials)
    print(f"  Comprehensive study has {total_trials} total trials")
    ...
    print(f"  - {contour_count} comprehensive contour maps in {contour_dir}")
    print(f"  - {slice_count} comprehensive slice plots in {slice_dir}")
    print(f"  - {parallel_count} comprehensive parallel plots in {parallel_dir}")
    print(f"  - {importance_count} comprehensive parameter importance plots in {importance_dir}")
    print(f"  - {history_count} comprehensive optimization history plots in {history_dir}")
    print(f"  - Total trials analyzed: {total_trials}")
    print(f"  - Data sources: 4 optimization runs (2025-05-26 to 2025-05-29)")
