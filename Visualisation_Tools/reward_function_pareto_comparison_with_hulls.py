...
Each reward function type gets its own color, Pareto frontier, and convex hull region.
"""
...
def create_reward_function_pareto_comparison_with_hulls():
    """Create Pareto frontier comparison plot for reward function types with convex hull regions."""
    ...
    # First, plot convex hull regions for each reward function type
    for reward_type, metrics in all_metrics.items():
        if not metrics:
            continue
        ...
        if len(x_coords) >= 3:
            ...
            try:
                ...
                ax.fill(hull_points[:, 0], hull_points[:, 1], 
                       ...
                       label=f'{reward_type} region')
            except Exception as e:
                print(f"Could not create convex hull for {reward_type}: {e}")
    
    # Then plot each reward function type
    for reward_type, metrics in all_metrics.items():
        if not metrics:
            continue
        ...
        ax.scatter(x_coords, y_coords, 
                   c=[color], alpha=0.8, s=65, 
                   label=f'{reward_type} (n={len(metrics)})',
                   edgecolors='white', linewidth=0.5, zorder=10)
        
        print(f"{reward_type}: {len(metrics)} agents")
        print(f"  Goal reached range: {min(x_coords):.3f} - {max(x_coords):.3f}")
        print(f"  1-Lava proportion range: {min(y_coords):.3f} - {max(y_coords):.3f}")
    ...
    ax.set_title('Pareto Frontier Comparison: Reward Function Types (with Convex Hull Regions)', fontsize=16, fontweight='bold')
    ...
    # Create legend with only the scatter point labels (not the region labels)
    ...
    ax.legend(handles=point_handles, labels=point_labels, fontsize=12, loc='best')
    ...
    # Save the plot
    save_path = os.path.join(save_dir, "reward_function_pareto_comparison_with_hulls.png")
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"\nReward function Pareto comparison plot with convex hulls saved: {save_path}")
...
if __name__ == "__main__":
    print("Generating reward function Pareto frontier comparison with convex hull regions...")
    ...
    print("Regions: Convex hull areas for each reward function type\n")
    
    save_path = create_reward_function_pareto_comparison_with_hulls()
    
    if save_path:
        print("\nReward function Pareto comparison with convex hulls completed!")
    else:
        print("Failed to generate reward function Pareto comparison plot with convex hulls.") 