...
class IndividualOptunaPlotExtractor:
    ...
    def __init__(self, results_file_path: str, config_file_path: str = None, output_dir: str = None):
        ...
        print(f"Created subfolders in {self.output_dir}:")
        print(f"  - {self.contour_dir}")
        print(f"  - {self.slice_dir}")
        print(f"  - {self.parallel_dir}")
    ...
    def extract_individual_contour_maps(self):
        """Extract individual contour maps for each parameter pair with minimal point obstruction."""
        print("\nExtracting individual contour maps...")
        ...
        for obj_name, obj_func, obj_title in objectives:
            for param1, param2 in combinations(param_names, 2):
                try:
                    ...
                    # Make points much smaller and more transparent to not obscure contours
                    fig.update_traces(
                        marker=dict(size=1, opacity=0.2),  # Very small, transparent points
        ...
        print(f"Generated {count} individual contour maps")
    ...
    def extract_individual_slice_plots(self):
        ...
        print(f"Generated {count} individual slice plots")
    ...
    def create_blown_up_parallel_plots(self):
        ...
        for obj_name, obj_func, obj_title in objectives:
            try:
                ...
                fig.update_layout(
                    title=f"Blown-Up Parallel Coordinate Plot: {obj_title}",
                    width=1600,  # Very wide
                    height=1000,  # Very tall
                    font=dict(size=16),  # Large font
        ...
        print(f"Generated {count} blown-up parallel coordinate plots")
    ...
    def extract_all_individual_plots(self):
        ...
        print(f"\n" + "="*50)
        print(f"✓ Individual plot extraction complete!")
        ...
        print(f"Summary:")
        print(f"  - {contour_count} individual contour maps in {self.contour_dir}")
        print(f"  - {slice_count} individual slice plots in {self.slice_dir}")
        print(f"  - {parallel_count} blown-up parallel plots in {self.parallel_dir}")
...
def main():
    ...
    # Use the direct relative path that works
